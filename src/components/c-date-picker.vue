...
<script lang="ts">
  ...
  import eDate from '../src/components/_e-date.vue';

  interface ISetup {
    field: Ref<HTMLInputElement>;
    calendar: Ref<HTMLDivElement>;
  }
  interface IData {

    /**
     *  Holds the instance of the calendar widget.
     */
    calendarInstance: Pikaday | null;

    /**
     * Holds the visibility state of the calendar widget.
     */
    isCalendarVisible: boolean;

    /**
     * Holds the currently selected start date.
     */
    startDate: Date;

    /**
     * Holds the currently selected end date.
     */
    endDate: Date;

    /**
     * Determines if the start or end date is currently edited.
     */
    editRangeStart: boolean;

    /**
     * Holds the current close timeout id.
     */
    closeTimeout: ReturnType<typeof setTimeout> | null;

    /**
     * Holds the flag, if the calendar is currently in closing state.
     */
    isClosing: boolean;
  }
  ...
  export default defineComponent({
    ...
    props: {
       ...
       */
        ...
        type: Date,
        default() {
          // TODO:FIX return this.$dayjs().add(7, 'day').toDate();
          return new Date();
        },
      },
    ...
    watch: {
      ...
      startDate(value, oldValue) {
        ...
        if (this.range) {
          ...
          this.calendarInstance.draw(false);
        } else {
          this.close();
        }
    ...
    methods: {
      ...
      createCalendar() {
        ...
        if (this.range) {
          ...
          this.calendarInstance.draw(false); // Pickaday does not update automatically when setting the Range. This seems to be a bug.
        }
      },
       ...
       *
       */
      onCalendarDraw(picker: Pikaday): void {
        if (this.range) {
      ...
      close(delay?: boolean) {
        ...
        const elementInFocus: HTMLInputElement | null = document.querySelector(':focus');
